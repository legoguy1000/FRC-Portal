  ...
  public function getWeeklyBuildSeasonHoursAttribute() {
    ...
    if(isset($this->attributes['user_id']) && isset($this->attributes['season_id'])) {
      $hours = DB::table('meeting_hours')
              ...
              ->whereRaw('seasons.season_id = "'.$this->attributes['season_id'].'"')
              ->whereRaw('meeting_hours.user_id = "'.$this->attributes['user_id'].'"')
              ->select(DB::raw('user_id, seasons.start_date, @week_hours := SUM(time_to_sec(IFNULL(timediff(meeting_hours.time_out, meeting_hours.time_in),0)) / 3600) as week_hours, week(meeting_hours.time_in,1) as week, (@week_hours >= 1) >= seasons.hour_requirement_week) as req_complete'))->groupBy('meeting_hours.user_id', 'week')->get();
    }
    if(!is_null($hours)) {
      $first = new DateTime();
