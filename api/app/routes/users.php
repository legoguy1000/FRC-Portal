...
$app->group('/users', function () {
  $this->get('', function ($request, $response, $args) {
    ...
    $queryArr = array();
      ...
      $users = FrcPortal\User::leftJoin('schools', 'users.school_id', '=', 'schools.school_id')->select('schools.school_name', 'schools.abv')->havingRaw($queryStr)->get();
    ...
    if($filter != '' ) {
      $users = FrcPortal\User::with('school')->leftJoin('schools', 'users.school_id', '=', 'schools.school_id')->select('schools.school_name', 'schools.abv')->havingRaw($queryStr)->orderBy($orderCol,$orderBy)->offset($offset)->limit($limit)->get();
    } else {
