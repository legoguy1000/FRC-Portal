    ...
    $this->group('/eventRequirements', function () {
      ...
      $this->group('/{event_id:[a-z0-9]{13}}', function () {
        $this->get('', function ($request, $response, $args) {
          ...
          $user = FrcPortal\Event::with(['event_rooms', 'event_time_slots', 'event_requirements.event_cars', 'event_requirements.event_rooms.users', 'event_requirements.event_time_slots', 'event_requirements' => function ($query) use ($user_id) {
                    $query->where('user_id',$user_id); // fields from comments table,
