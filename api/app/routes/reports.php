...
$app->group('/reports', function () {
  $this->get('/avgHrsPerPersonPerYear', function ($request, $response, $args) {
    ...
    if(!isset($request->getParam('start_date')) || $request->getParam('start_date') == '' || !is_numeric($request->getParam('start_date'))) {
        $responseArr = array('status'=>false, 'msg'=>'Invalid Start Date.');
        $response = $response->withJson($responseArr,400);
        return $response;
    }
    if(!isset($request->getParam('end_date')) || $request->getParam('end_date') == '' || !is_numeric($request->getParam('end_date'))) {
