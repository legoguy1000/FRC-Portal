...
$query = 'SELECT COUNT(DISTINCT(m.user_id)) as user_count, YEAR(m.time_in) as year, u.user_type FROM meeting_hours m LEFT JOIN users u USING(user_id) WHERE u.user_type <> ""  GROUP BY year,u.user_type';
...
$query = 'SELECT COUNT(DISTINCT(m.user_id)) as user_count, YEAR(m.time_in) as year, u.gender FROM meeting_hours m LEFT JOIN users u USING(user_id) WHERE u.gender <> "" GROUP BY year,u.gender';
$result = db_select($query);
foreach($result as $re) {
	$gender = $re['gender'];
	$year = (integer) $re['year'];
