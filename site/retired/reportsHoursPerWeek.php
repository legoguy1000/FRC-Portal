...
(SELECT IFNULL(SUM(time_to_sec(timediff(mh.time_out, mh.time_in)) / 3600),0) as hours, week(mh.time_in,1) as week from meeting_hours mh WHERE year(mh.time_in)='.db_quote($year).' GROUP BY week) a
GROUP BY week';
//die($query);
...
$allData = array(
	...
	'series' => $series,
	'data' => $data,
	'csvData' => metricsCreateCsvData($data, $labels, $series)
);
